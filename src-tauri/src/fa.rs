...
async fn check_e621_md5(client: &reqwest::Client, hash: &str) -> Option<E621Post> {
    ...
    match client.get(&url).header("User-Agent", "TailBurrow/0.2.3").send().await {
        Ok(resp) => {
...
async fn run_sync_inner(
    ...
    loop {
        if *state.should_cancel.lock().map_err(|e| e.to_string())? {
            break;
        }
        ...
        for id_str in ids {
                ...
                .await
            {
                Ok(r) => r,
                Err(e) => {
                    eprintln!("Failed to fetch FA view page {}: {}", id_str, e);
                    state.status.lock().map_err(|e| e.to_string())?.errors += 1;
                    continue;
                }
            };
            ...
            let (download_url, fa_tags, artist_name, rating_char) = {
                ...
                let dl = match view_doc.select(&download_selector).next() {
                    Some(el) => {
                        Some(format!("https:{}", el.value().attr("href").unwrap_or("")))
                    }
                    None => None,
                };

                let tag_selector =
                    Selector::parse("section.tags-row span.tags a").unwrap();
                let tags: Vec<String> = view_doc
                    .select(&tag_selector)
            ...
            if let Some(e621_post) = check_e621_md5(&e621_client, &hash_str).await {
                ...
                if let Some(file_url) = e621_post.file.url {
                    let e621_bytes = match e621_client
                        .get(&file_url)
                        .header("User-Agent", "TailBurrow/0.2.3")
                        .send()
                        .await
                    {
                        Ok(r) => match r.bytes().await {
